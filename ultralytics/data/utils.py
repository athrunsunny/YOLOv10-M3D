...
def _load_calibration(calib_file):
    ...
    return {
        'P2': P2,
        'intrinsics': copy.deepcopy(P2)[:3, :3]
    }


def verify_image_label(args: tuple) -> list:
    """Verify one image-label pair."""
    im_file, lb_file, cal_file, prefix, keypoint, num_cls, nkpt, ndim, single_cls, class_map = args
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    points_3d = [] if class_map is not None else None
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file, encoding="utf-8") as f:
                ...
                if any(len(x) > 6 for x in lb) and (not keypoint) and (not class_map):  # is segment
                ...
                if class_map is not None:
                    for item in lb:
                        if class_map[item[0]] < 0:
                            continue
                        item[0] = str(class_map[item[0]])
                        data = item[4:8]
                        x1 ,y1, x2, y2 = map(lambda x: float(x), data)
                        ...
                        item[4:8] = [str(cx), str(cy), str(w), str(h)]
                        new_lb.append(item)
                    lb = new_lb
                    classes = np.array([x[0] for x in lb], dtype=np.float32)
                    points_3d = np.array([x[8:] for x in lb], dtype=np.float32)
                    points_2d = np.array([x[4:8] for x in lb], dtype=np.float32)
                    lb = np.concatenate((classes.reshape(-1, 1), points_2d), 1)
                if cal_file is not None:
                    if os.path.isfile(cal_file):
                        calib = _load_calibration(cal_file)
                        # ✅ FIX: Scale camera intrinsics to match resized image
                        if 'intrinsics' in calib:
                            calib['intrinsics'][0, 0] /= shape[1]  # fx
                            calib['intrinsics'][1, 1] /= shape[0]  # fy
                            calib['intrinsics'][0, 2] /= shape[1]  # cx
                            calib['intrinsics'][1, 2] /= shape[0]  # cy

                        # ✅ FIX: Also scale P2 matrix (projection matrix)
                        if 'P2' in calib:
                            calib['P2'][0, :] /= shape[1]  # Scale first row (x-related)
                            calib['P2'][1, :] /= shape[0]  # Scale second row (y-related)
                        intrinsics = calib['intrinsics'].reshape(-1)

                        new_intrinsics = np.tile(intrinsics, (len(lb), 1))
                        points_3d = np.concatenate((points_3d, new_intrinsics), axis=1)
            ...
            if nl := len(lb):
                ...
                if len(i) < nl:  # duplicate row check
                    ...
                    if points_3d:
                        points_3d = [points_3d[x] for x in i]
        ...
        lb = lb[:, :5]
        return im_file, lb, shape, segments, keypoints, points_3d, nm, nf, ne, nc, msg
    except Exception as e:
        nc = 1
        msg = f"{prefix}{im_file}: ignoring corrupt image/label: {e}"
