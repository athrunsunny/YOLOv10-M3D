...
class Results(SimpleClass, DataExportMixin):
    ...
    def __init__(
        ...
        self.points_3d = Boxes3D(points_3d, self.orig_shape) if points_3d is not None else None
        self.obb = OBB(obb, self.orig_shape) if obb is not None else None
        self.speed = speed if speed is not None else {"preprocess": None, "inference": None, "postprocess": None}
        self.names = names
        self.path = path
        self.save_dir = None
        self._keys = "boxes", "masks", "probs", "keypoints", "obb", "points_3d"
    ...
    def update(
        ...
        if points_3d is not None:
            self.points_3d = Boxes3D(points_3d, self.orig_shape)
    ...
    def plot(
        ...
        if self.points_3d is not None and calib is not None:
            import cv2
            from ultralytics.utils.plotting import get_3d_box_corners, project_to_image, draw_3d_box
            intrinsics = calib['intrinsics']
            im = np.asarray(annotator.im).copy()
            for i, k in enumerate(reversed(self.points_3d.data)):
                dims = k[:3].cpu().numpy().tolist()
                locs = k[3:6].cpu().numpy().tolist()
                yaw = float(k[6].cpu().item())
                corners_3d = get_3d_box_corners(dims, locs, yaw)  # (8, 3)
                corners_2d = project_to_image(corners_3d, intrinsics)

                clip_margin = 200
                # if np.all((corners_2d[:, 0] > -clip_margin) & (corners_2d[:, 0] < im.shape[1] + clip_margin) &
                #           (corners_2d[:, 1] > -clip_margin) & (corners_2d[:, 1] < im.shape[0] + clip_margin)):
                cls_idx = int(pred_boxes[i].cls.item())
                color = colors(cls_idx if cls_idx is not None else 0)
                im = draw_3d_box(im, corners_2d, color, thickness=2, cls=cls_idx)
                center = corners_2d.mean(axis=0).astype(int)
                label_text = f"{names.get(cls_idx, cls_idx) if (names and cls_idx is not None) else ''}"
                depth_text = f"{locs[2]:.1f}m"
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                ...
                if boxes3ds is not None:
                    line = (c, 0.0, 0.0, 0.0, *(d.xyxy.view(-1))) # rewrite line
                    boxes3d = boxes3ds[j]
                    line += (*boxes3d.data.reshape(-1), )
                line += (conf,) * save_conf + (() if id is None else (id,))
                texts.append(("%g " * len(line)).rstrip() % line)
                if boxes3ds is not None:
                    texts[j] = boxes3ds.cls[int(c)] + texts[j][1:]
...
class Boxes3D(BaseTensor):
    ...
    def __init__(self, boxes: torch.Tensor | np.ndarray, orig_shape: tuple[int, int]) -> None:
        ...
        if boxes.ndim == 1:
            boxes = boxes[None, :]
        n = boxes.shape[-1]
        assert n in {6, 7}, f"expected 6 or 7 values but got {n}"  # h, w, l, x, y, z, yaw
        super().__init__(boxes, orig_shape)
        self.orig_shape = orig_shape

    @property
    def dim(self) -> torch.Tensor | np.ndarray:
        """Return bounding boxes in [x1, y1, x2, y2] format.
    ...
    @property
    def z(self) -> torch.Tensor | np.ndarray:
        """Return the confidence scores for each detection box.
    ...
    @property
    def yaw(self) -> torch.Tensor | np.ndarray:
        """Return the class ID tensor representing category predictions for each bounding box.
    ...
    @property
    def loc(self) -> torch.Tensor | np.ndarray | None:
        """Return the tracking IDs for each detection box if available.
