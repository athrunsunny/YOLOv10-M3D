...
class YOLODataset(BaseDataset):
    ...
    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        ...
        self.use_detect3d = task == "detect3d"
        self.data = data
        # if 'mode' in kwargs:
        self.mode = kwargs.pop('mode')
        assert not (self.use_segments and self.use_keypoints), "Can not use both segments and keypoints."
        super().__init__(*args, channels=self.data.get("channels", 3), **kwargs) # ultralytics/data/base.py(72)

    def _get_image_ids(self):
        """
        Get list of image IDs from KITTI ImageSets split files (MonoLSS standard)

        MonoLSS uses ImageSets/{split}.txt files that define the exact train/val split.
        This ensures consistent splits across different implementations.

        Falls back to using all images if ImageSets file doesn't exist.
        """
        if os.path.exists(self.imageset_dir):
            # Read from ImageSets file (standard KITTI split)
            with open(self.imageset_dir, 'r') as f:
                image_ids = [line.strip() for line in f.readlines() if line.strip()]
            print(f"[INFO] Loaded {len(image_ids)} images from {self.imageset_dir}")
            return image_ids
        else:
            # Fallback: list all images and potentially split them
            print(f"[WARNING] ImageSets file not found: {self.imageset_dir}")
            print(f"[WARNING] Using fallback: listing all images from {self.image_dir}")

            image_files = sorted(os.listdir(self.image_dir))
            all_image_ids = [f.replace('.png', '') for f in image_files if f.endswith('.png')]

            # If train/val split needed but no ImageSets file, use 80/20 split as fallback
            if self.split in ['train', 'val']:
                print(f"[WARNING] Using 80/20 split as fallback (seed=42)")
                np.random.seed(42)
                n_train = int(0.8 * len(all_image_ids))

                if self.split == 'train':
                    return all_image_ids[:n_train]
                else:  # val
                    return all_image_ids[n_train:]
            else:
                # test or trainval: use all images
                return all_image_ids

    def _get_image_labels_from_image_id(self):
        assert len(self.label_files) == len(self.im_files), f'Found {len(self.label_files)} label files == {len(self.im_files)} image files'
        new_labels = []
        new_im_files = []
        new_calib_files = []
        for lb, im in zip(self.label_files, self.im_files):
            lb_stem = Path(lb).stem
            im_stem = Path(im).stem
            assert lb_stem == im_stem, f'Label file {lb} and image file {im} do not match'
            if lb_stem in self.kitti3d_id:
                lb_parts = list(Path(lb).parent.parts)
                lb_parts[-1] = lb_parts[-1].replace('image_2', 'label_2')
                lb = Path(*lb_parts)
                lb = os.path.join(lb, lb_stem + '.txt')
                new_labels.append(lb)
                new_im_files.append(im)
                calib_file = lb.replace('label_2', 'calib')
                new_calib_files.append(calib_file)
        self.label_files = new_labels
        self.im_files = new_im_files
        self.calib_files = new_calib_files

    def _ge_class_dict(self):
        self.CLASSES = ['Car', 'Pedestrian', 'Cyclist']

        # Class mapping for KITTI (following MonoLSS and standard practice)
        # Maps similar classes together to increase training data
        self.CLASS_MAPPING = {
            'Car': 0,
            'Van': 0,  # Merge Van -> Car
            'Truck': 0,  # Merge Truck -> Car
            'Pedestrian': 1,
            'Person_sitting': 1,  # Merge Person_sitting -> Pedestrian
            'Cyclist': 2,
            'Tram': -1,  # Ignore
            'Misc': -1,  # Ignore
            'DontCare': -2  # Special: used in evaluation to avoid FP
        }
    ...
    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict:
        ...
        if not self.use_detect3d:
            self.CLASS_MAPPING = None

        self.calib_files = getattr(self, 'calib_files', None)
        if not self.calib_files:
            self.calib_files = [None] * len(self.im_files)
        ...
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(
                ...
                iterable=zip(
                    ...
                    repeat(self.CLASS_MAPPING)
                ),
            )
            pbar = TQDM(results, desc=desc, total=total)
            for im_file, lb, shape, segments, keypoint, points_3d, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
    ...
    def get_labels(self) -> list[dict]:
        ...
        if self.use_detect3d:
            ImageSets_path = Path(self.img_path).parent.parent / 'ImageSets'
            self.imageset_dir = ImageSets_path / f'{self.mode}.txt'
            self.kitti3d_id = self._get_image_ids()
            self._get_image_labels_from_image_id()
            self._ge_class_dict()
            cache_path = Path(self.label_files[0]).parent / f'{self.mode}.cache'
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        if self.augment:
            ...
            if not self.use_detect3d:
                ...
                transforms = v8_transforms3d(self, self.imgsz, hyp)
        else:
        ...
        transforms.append(
            Format(
                ...
                return_points_3d=self.use_detect3d,
    ...
    def update_labels_info(self, label: dict) -> dict:
        ...
        points_3d = label.pop("points_3d", None)
        ...
        label["instances"] = Instances(bboxes, segments, keypoints, points_3d, bbox_format=bbox_format, normalized=normalized)
        return label
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb", "points_3d"}:
