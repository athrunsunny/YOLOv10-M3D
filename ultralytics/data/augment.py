...
class RandomPerspective:
    ...
    def apply_bboxes3d(self, points3d: np.ndarray, M: np.ndarray) -> np.ndarray:
        """Apply affine transformation to bounding boxes.
        ...
        Examples:
            >>> points3d = torch.tensor([[10, 10, 20, 20, 30, 30, 40]])
            >>> M = torch.eye(3)
            >>> transformed_bboxes = apply_bboxes3d(points3d, M)
        """
        n = len(points3d)
        if n == 0:
            return points3d
        ...
        xyz = points3d[:, 3:6]
        yaw = points3d[:, 6:7]
        intrinsics = points3d[:, 7:]

        # h = hwl[:, 0:1] / 2.0
        xyz[:, 2:3] = xyz[:, 2:3] #-h
        fx = intrinsics[:, 0:1] * img_w
        fy = intrinsics[:, 5:6] * img_h
        cx = intrinsics[:, 2:3] * img_w
        cy = intrinsics[:, 6:7] * img_h

        tx = intrinsics[:, 3:4] * img_w / -fx
        ty = intrinsics[:, 7:8] * img_h / -fy

        z = xyz[:, 2:3].copy()
        z[z < 1e-6] = 1e-6

        u = (xyz[:, 0:1] - tx) * fx / z + cx
        v = (xyz[:, 1:2] - ty) * fy / z + cy
        center_2d = np.concatenate([u, v], axis=1)

        xy = np.ones((n, 3), dtype=points3d.dtype)
        xy[:, :2] = center_2d
        xy = xy @ M.T
        center_2d_transformed = (xy[:, :2] / xy[:, 2:3] if self.perspective else xy[:, :2])

        scale_x = np.sqrt(M[0, 0] ** 2 + M[0, 1] ** 2)
        scale_y = np.sqrt(M[1, 0] ** 2 + M[1, 1] ** 2)
        avg_scale = (scale_x + scale_y) / 2

        hwl_transformed = hwl * avg_scale

        u_transformed = center_2d_transformed[:, 0:1]
        v_transformed = center_2d_transformed[:, 1:2]

        x_transformed = (u_transformed - cx) * z / fx + tx
        y_transformed = (v_transformed - cy) * z / fy + ty
        xyz_transformed = np.concatenate([x_transformed, y_transformed, z], axis=1)

        # h_transformed = hwl_transformed[:, :1] / 2
        xyz_transformed[:, 1:2] = xyz_transformed[:, 1:2] # + h_transformed
        ...
        points3d_transformed = np.concatenate([hwl_transformed, xyz_transformed, yaw_transformed, intrinsics], axis=1)
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if points_3d is not None:
            points_3d = self.apply_bboxes3d(points_3d, M)

        new_instances = Instances(bboxes, segments, keypoints, points_3d, bbox_format="xyxy", normalized=False)
        # Clip
...
class RandomFlip:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.direction == "vertical" and random.random() < self.p:
            ...
            if self.flip_idx is not None and instances.points_3d is not None:
                instances.points_3d = self.apply_points3d_flip(instances.points_3d, img)
    ...
    def apply_points3d_flip(self, points3d: np.ndarray, img: np.ndarray) -> np.ndarray:
        if len(points3d) == 0:
            return points3d

        points3d_transformed = points3d.copy()
        hwl = points3d_transformed[:, :3]
        xyz = points3d_transformed[:, 3:6]
        yaw = points3d_transformed[:, 6:7]
        intrinsics = points3d_transformed[:, 7:]

        img_h, img_w = img.shape[:2]
        intrinsics[:, [0, 2, 3]] *= img_w
        intrinsics[:, [5, 6, 7]] *= img_h

        if self.direction == 'horizontal':
            intrinsics[:, 2:3] = img_w - intrinsics[:, 2:3]
            yaw_transformed = np.pi - yaw
            # yaw_transformed = (yaw_transformed + np.pi) % (2 * np.pi) - np.pi
            points3d_transformed[:, 6:7] = yaw_transformed
            xyz[:, :1] *= -1

        elif self.direction == 'vertical':
            intrinsics[:, 6:7] = img_h - intrinsics[:, 6:7]
            yaw_transformed = np.pi - yaw
            # yaw_transformed = (yaw_transformed + np.pi) % (2 * np.pi) - np.pi
            points3d_transformed[:, 6:7] = yaw_transformed
            xyz[:,1:2] *= -1
        ...
        intrinsics[:, [0, 2, 3]] /= img_w
        intrinsics[:, [5, 6, 7]] /= img_h
        points3d_transformed[:, 0:3] = hwl
        points3d_transformed[:, 3:6] = xyz
        points3d_transformed[:, 7:] = intrinsics
        return points3d_transformed
...
class Format:
    ...
    def __init__(
        ...
        self.return_points_3d = return_points_3d
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.return_points_3d:
            # labels["points_3d"] = torch.from_numpy(instances.points_3d) if instances.points_3d is not None else torch.zeros((0, 16))
            labels["points_3d"] = torch.from_numpy(
                instances.points_3d) if instances.points_3d is not None else torch.zeros(
                (0, 16))  # h w l x y z yaw intrinsics
...
def v8_transforms3d(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    """Apply a series of image transformations for training.
    ...
    return Compose(
        [
            # MixUp3D(dataset, pre_transform=None, p=0.5),
            RandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
            Albumentations(p=1.0, transforms=getattr(hyp, "augmentations", None)),
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            RandomCrop3D(size=None, p=0.5, scale_range=(0.7, 1.0)),
        ]
    )  # transforms


class RandomCrop3D:
    def __init__(self, size: tuple[int, int] = None, p: float = 1.0, scale_range: tuple[float, float] = (0.5, 1.5)):
        assert 0 <= p <= 1.0, f'The probability should be in range [0, 1], but got {p}'
        self.size = size
        self.p = p
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        valid = (bboxes[:, 2] > bboxes[:, 0]) & (bboxes[:, 3] > bboxes[:, 1]) & \
                (bboxes[:, 2] - bboxes[:, 0] > 1) & (bboxes[:, 3] - bboxes[:, 1] > 1)

        segments = instances.segments
        keypoints = instances.keypoints
        points_3d = instances.points_3d

        new_bboxes = bboxes[valid]

        new_segments = instances.segments[valid] if len(segments) else segments
        new_keypoints = instances.keypoints[valid] if keypoints is not None else keypoints

        if len(points_3d):
            new_points_3d, new_image = self.apply_points3d_crop(instances.points_3d, x1, y1, img, img_cropped)
            new_points_3d = new_points_3d[valid]
            new_img_h, new_img_w = new_image.shape[:2]
            new_bboxes[:, [0, 1]] = new_bboxes[:, [0, 1]] / crop_w * new_img_w
            new_bboxes[:, [2, 3]] = new_bboxes[:, [2, 3]] / crop_h * new_img_h
            img = new_image
        else:
            new_points_3d = points_3d

        new_instances = Instances(new_bboxes, new_segments, new_keypoints, new_points_3d, bbox_format="xyxy", normalized=False)
    ...
    def apply_points3d_crop(self, points3d: np.ndarray, x1: int, y1: int, ori_img: np.ndarray, crop_img: np.ndarray) -> np.ndarray:
        if len(points3d) == 0:
            return points3d, ori_img
        points3d_transformed = points3d.copy()
        hwl = points3d_transformed[:, :3]
        xyz = points3d_transformed[:, 3:6]
        yaw = points3d_transformed[:, 6:7]
        ...
        intrinsics[:, [0, 2, 3]] /= crop_w
        intrinsics[:, [5, 6, 7]] /= crop_h

        points3d_transformed[:, 7:] = intrinsics

        new_image = cv2.resize(crop_img, (img_w, img_h))
        return points3d_transformed, new_image


class MixUp3D(BaseMixTransform):
    """Base class for mix transformations like Cutmix, MixUp and Mosaic.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        mix_points3d = mix_labels[0]['instances'].points_3d
        points3d = labels['instances'].points_3d

        try:
            if len(mix_points3d) and len(points3d):
                calib = labels['instances'].points_3d[0, 7:]
                mix_calib = mix_labels[0]['instances'].points_3d[0, 7:]
                if not np.array_equal(calib, mix_calib):
                    return labels
        except:
            return labels
