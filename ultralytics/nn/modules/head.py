...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect", "v10Detect3D"
...
class v10Detect3Dbase(Detect):
    """v10 Detection3D head from https://arxiv.org/pdf/2405.14458.
    ...
    Methods:
        __init__: Initialize the v10Detect3D object with specified number of classes and input channels.
        forward: Perform forward pass of the v10Detect3D module.
        bias_init: Initialize biases of the Detect module.
        fuse: Remove the one2many head for inference optimization.

    Examples:
        Create a v10Detect3D head
        >>> v10_detect = v10Detect(nc=80, ch=(256, 512, 1024))
        >>> x = [torch.randn(1, 256, 80, 80), torch.randn(1, 512, 40, 40), torch.randn(1, 1024, 20, 20)]
        >>> outputs = v10_detect(x)
    """
...
class v10Detect3D(Detect):
    ...
    def __init__(self, nc: int = 80, ch: tuple = ()):
        ...
        # cv2 is 2D Box Regression Head
        # cv3 is Classification Head

        # ===== 3D Regression Heads =====
        # Depth + Uncertainty (2 channels)
        self.c_depth = 2
        self.cv_depth = nn.ModuleList(nn.Sequential(nn.Sequential(Conv(x, x, 3, g=x), Conv(x, c3, 1)), \
                                                    nn.Sequential(Conv(c3, c3, 3, g=c3), Conv(c3, c3, 1)), \
                                                    nn.Conv2d(c3, self.c_depth, 1)) for i, x in enumerate(ch))

        # 3D Dimensions (3 channels: h, w, l offset)
        self.c_dim = 3
        self.cv_dim = nn.ModuleList(nn.Sequential(nn.Sequential(Conv(x, x, 3, g=x), Conv(x, c3, 1)), \
                                                  nn.Sequential(Conv(c3, c3, 3, g=c3), Conv(c3, c3, 1)), \
                                                  nn.Conv2d(c3, self.c_dim, 1)) for i, x in enumerate(ch))

        # Orientation (Conf + Sin + Cos per bin -> 3 * bins)
        num_ori_bins = 12
        self.c_bins = num_ori_bins * 3
        self.cv_ori = nn.ModuleList(nn.Sequential(nn.Sequential(Conv(x, x, 3, g=x), Conv(x, c3, 1)), \
                                                  nn.Sequential(Conv(c3, c3, 3, g=c3), Conv(c3, c3, 1)), \
                                                  nn.Conv2d(c3, self.c_bins, 1)) for i, x in enumerate(ch))

        # 3D Offset (2 channels: projected center offset x, y)
        self.c_off = 2
        self.cv_offset = nn.ModuleList(nn.Sequential(nn.Sequential(Conv(x, x, 3, g=x), Conv(x, c3, 1)), \
                                                     nn.Sequential(Conv(c3, c3, 3, g=c3), Conv(c3, c3, 1)), \
                                                     nn.Conv2d(c3, self.c_off, 1)) for i, x in enumerate(ch))

        self.one2one_cv2 = copy.deepcopy(self.cv2)
        self.one2one_cv3 = copy.deepcopy(self.cv3)
        self.one2one_cv_depth = copy.deepcopy(self.cv_depth)
        self.one2one_cv_dim = copy.deepcopy(self.cv_dim)
        self.one2one_cv_ori = copy.deepcopy(self.cv_ori)
        self.one2one_cv_offset = copy.deepcopy(self.cv_offset)
        # self.one2one_cv3 = copy.deepcopy(self.cv3)

    def fuse(self):
        """Remove the one2many head for inference optimization."""
        self.cv2 = self.cv3 = None
        self.cv_depth = self.cv_dim = self.cv_ori = self.cv_offset = None

    @property
    def one2many(self):
        """Returns the one-to-many head components, here for v5/v5/v8/v9/11 backward compatibility."""
        return dict(box_head=self.cv2, cls_head=self.cv3,
                    depth_head=self.cv_depth, dim_head=self.cv_dim,
                    ori_head=self.cv_ori, offset_head=self.cv_offset)

    @property
    def one2one(self):
        """Returns the one-to-one head components."""
        return dict(box_head=self.one2one_cv2, cls_head=self.one2one_cv3,
                    depth_head=self.one2one_cv_depth, dim_head=self.one2one_cv_dim,
                    ori_head=self.one2one_cv_ori, offset_head=self.one2one_cv_offset)
    ...
    def forward_head(
        self, x: list[torch.Tensor], box_head: torch.nn.Module = None, cls_head: torch.nn.Module = None,
            depth_head: torch.nn.Module = None, dim_head: torch.nn.Module = None,
            ori_head: torch.nn.Module = None, offset_head: torch.nn.Module = None
    ) -> dict[str, torch.Tensor]:
        """Concatenates and returns predicted bounding boxes and class probabilities."""
        if box_head is None or cls_head is None or depth_head is None or dim_head is None or ori_head is None or offset_head is None:  # for fused inference
            return dict()
        bs = x[0].shape[0]  # batch size
        boxes = torch.cat([box_head[i](x[i]).view(bs, 4 * self.reg_max, -1) for i in range(self.nl)], dim=-1)
        scores = torch.cat([cls_head[i](x[i]).view(bs, self.nc, -1) for i in range(self.nl)], dim=-1)
        depths = torch.cat([depth_head[i](x[i]).view(bs, self.c_depth, -1) for i in range(self.nl)], dim=-1)
        dims = torch.cat([dim_head[i](x[i]).view(bs, self.c_dim, -1) for i in range(self.nl)], dim=-1)
        orientations = torch.cat([ori_head[i](x[i]).view(bs, self.c_bins, -1) for i in range(self.nl)], dim=-1)
        offsets = torch.cat([offset_head[i](x[i]).view(bs, self.c_off, -1) for i in range(self.nl)], dim=-1)
        return dict(boxes=boxes, scores=scores,
                    depths=depths, dims=dims, orientations=orientations, offsets=offsets,
                    feats=x)
    ...
    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        depths = x['depths']
        dims = x['dims']
        orientations = x['orientations']
        offsets = x['offsets']

        # 3D center offset
        y_off = offsets.clone()
        ndim = offsets.shape[1]
        y_off[:, 0::ndim] = (y_off[:, 0::ndim] * 2.0 + (self.anchors[0] - 0.5)) * self.strides
        y_off[:, 1::ndim] = (y_off[:, 1::ndim] * 2.0 + (self.anchors[1] - 0.5)) * self.strides

        # depth
        dep = depths.clone()[:, :1, :]

        # rot
        ori = orientations.clone().permute(0, 2, 1)
        pred_ori = ori.view(ori.shape[0], ori.shape[1], 12, 3)
        ori_conf = pred_ori[..., 0]
        ori_res = pred_ori[..., 1:]
        best_bin = ori_conf.argmax(dim=-1)
        ...
        return torch.cat((dbox, x["scores"].sigmoid(), dims, y_off, dep, rotations), 1)
    ...
    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            (torch.Tensor): Processed predictions with shape (batch_size, min(max_det, num_anchors), 6) and last
                dimension format [x, y, w, h, max_class_prob, class_index].
        """
        preds_2d = preds.clone()[:, :, :4+self.nc]
        boxes, scores = preds_2d.split([4, self.nc], dim=-1)
        scores, conf, idx = self.get_topk_index(scores, self.max_det)
        boxes = boxes.gather(dim=1, index=idx.repeat(1, 1, 4))

        preds_3d = preds.clone()[:, :, 4 + self.nc:]
        preds_3d = preds_3d.gather(dim=1, index=idx.repeat(1, 1, preds_3d.shape[-1]))

        cls = conf.long().squeeze(-1)
        pred_dim = preds_3d[:, :, :3]
        mean_dims_tensor = torch.tensor(list(self.KITTI_MEAN_DIMS.values()), dtype=torch.float32, device=preds.device)
        mean_dim = mean_dims_tensor[cls]
        pred_dim = pred_dim + mean_dim
        preds_3d[:, :, :3] = pred_dim

        # pred_ori = preds_3d[:, :, 5:41].view(preds_3d.shape[0], preds_3d.shape[1], 12, 3)
        # ori_conf = pred_ori[..., 0]
        # ori_res = pred_ori[..., 1:]
        # best_bin = ori_conf.argmax(dim=-1)
        #
        ...
        # rotations = (bin_center + res_angle).unsqueeze(-1)
        #
        # # anchor_centers = self.anchors.unsqueeze(0).expand(preds_3d.shape[0], -1, -1).permute(0, 2, 1)
        # # anchor_strides = self.strides.unsqueeze(0).expand(preds_3d.shape[0], -1, -1).permute(0, 2, 1)
        # #
        # # anchor_centers = anchor_centers.gather(dim=1, index=idx.repeat(1, 1, anchor_centers.shape[-1]))
        # # anchor_strides = anchor_strides.gather(dim=1, index=idx.repeat(1, 1, anchor_strides.shape[-1]))
        # # anchor_centers_pixel = anchor_centers * anchor_strides
        # #
        # # pred_offsets = preds_3d[:, :, 41:43]
        # # offset_pixel = pred_offsets * anchor_strides
        # # proj_uv = anchor_centers_pixel + offset_pixel
        # #
        # # lpreds_3d = torch.cat([proj_uv, pred_depth, pred_dim, rotations], dim=-1)
        #
        # anchor_centers = self.anchors.unsqueeze(0).expand(preds_3d.shape[0], -1, -1).permute(0, 2, 1)
        # anchor_strides = self.strides.unsqueeze(0).expand(preds_3d.shape[0], -1, -1).permute(0, 2, 1)
        #
        ...
        # anchor_strides = anchor_strides.gather(dim=1, index=idx.repeat(1, 1, anchor_strides.shape[-1]))
        #
        # y = preds_3d[:, :, 41:43].clone()
        # y = (y * 2.0 + (anchor_centers - 0.5)) * anchor_strides
        # lpreds_3d = torch.cat([pred_dim, y, pred_depth, rotations], dim=-1)
        # return torch.cat([boxes, scores, conf, lpreds_3d], dim=-1)
        return torch.cat([boxes, scores, conf, preds_3d], dim=-1)
    ...
    def bias_init(self):
        """Initialize Detect() biases, WARNING: requires stride availability."""
        for i, (a, b, c, d, e, f) in enumerate(zip(self.one2many["box_head"], self.one2many["cls_head"],
                                                   self.one2many["depth_head"], self.one2many["dim_head"],
                                                   self.one2many["ori_head"], self.one2many["offset_head"])):  # from
            a[-1].bias.data[:] = 2.0  # box
            b[-1].bias.data[: self.nc] = math.log(
                5 / self.nc / (640 / self.stride[i]) ** 2)  # cls (.01 objects, 80 classes, 640 img)
            c[-1].bias.data[: self.c_depth] = math.log(
                5 / self.c_depth / (640 / self.stride[i]) ** 2)  # cls (.01 objects, 80 classes, 640 img)
            d[-1].bias.data[: self.c_dim] = math.log(
                5 / self.c_dim / (640 / self.stride[i]) ** 2)  # cls (.01 objects, 80 classes, 640 img)
            e[-1].bias.data[: self.c_bins] = math.log(
                5 / self.c_bins / (640 / self.stride[i]) ** 2)  # cls (.01 objects, 80 classes, 640 img)
            f[-1].bias.data[: self.c_off] = math.log(
                5 / self.c_off / (640 / self.stride[i]) ** 2)  # cls (.01 objects, 80 classes, 640 img)
        if self.end2end:
            for i, (a, b, c, d, e, f) in enumerate(zip(self.one2one["box_head"], self.one2one["cls_head"],
                                                       self.one2one["depth_head"], self.one2one["dim_head"],
                                                       self.one2one["ori_head"], self.one2one["offset_head"])):  # from
                a[-1].bias.data[:] = 2.0  # box
                b[-1].bias.data[: self.nc] = math.log(
                    5 / self.nc / (640 / self.stride[i]) ** 2)  # cls (.01 objects, 80 classes, 640 img)
                c[-1].bias.data[: self.c_depth] = math.log(
