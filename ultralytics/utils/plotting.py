...
# =============add 3D bbox viz=================
def get_3d_box_corners(dims, loc, rot_y):
    ...
    cos, sin = np.cos(rot_y), np.sin(rot_y)
...
def project_to_image(corners_3d, K):
    ...
    pts_2d = K @ corners_3d.T
    pts_2d[:2, :] /= (pts_2d[2:3, :] + 1e-7)
    return pts_2d[:2, :].T


def draw_3d_box1(img, corners_2d, color, thickness=2):
    ...
    # Draw top face
    for i in range(4, 8):
        cv2.line(img, tuple(corners_2d[i]), tuple(corners_2d[4 + (i - 4 + 1) % 4]), color, thickness)
    # Draw vertical edges
    for i in range(4):
...
def draw_3d_box(image, qs, color=(0, 60, 255), thickness=1, conf=None, cls=-1):
    '''
...
def plot_images(
    ...
    for k in {"cls", "bboxes", "conf", "masks", "keypoints", "batch_idx", "images",
              "points_3d", "intrinsics", "points_3d_labels", "points_3d_conf"}:
        if k not in labels:
            continue
    ...
    # 3D label (h, w, l, x, y, z, yaw)
    intrinsics = labels.get("intrinsics", None)
    points_3d = labels.get("points_3d", np.zeros([1, 19]))
    points_3d_labels = labels.get("points_3d_labels", None)
    points_3d_conf = labels.get("points_3d_conf", None)
    ...
    if intrinsics is None:
        try:
            intrinsics = points_3d[:, 7:].reshape(points_3d.shape[0], 3, 4)
        except:
            intrinsics = points_3d[:, 7:].reshape(points_3d.shape[0], 3, 3)
        points_3d = points_3d[:, :7]
        intrinsics[..., 0, 0] *= images.shape[3]
        intrinsics[..., 1, 1] *= images.shape[2]
        intrinsics[..., 0, 2] *= images.shape[3]
        intrinsics[..., 1, 2] *= images.shape[2]
        try:
            intrinsics[..., 0, 3] *= images.shape[3]
            intrinsics[..., 1, 3] *= images.shape[2]
        except:
            pass
        intrinsics = intrinsics[..., :3, :3]

    if points_3d_labels is None:
        points_3d_labels = cls

    if points_3d_conf is None:
        points_3d_conf = confs
        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels_flag or conf[j] > conf_thres:
                        label = f"{c}" if labels_flag else f"{c} {conf[j]:.1f}"
                ...
                for c in classes:
                    color = colors(c)
                    c = names.get(c, c) if names else c
                    label = f"{c}" if labels_flag else f"{c} {conf[0]:.1f}"
            ...
            #---------------------------3D visualization-----------------------------
            if isinstance(points_3d, np.ndarray) and points_3d.size:
                try:
                    if points_3d.shape[0] == batch_idx.shape[0]:
                        pts3d_idx = points_3d[idx]
                        pts3d_labels = points_3d_labels[idx] if points_3d_labels is not None else None
                        pts3d_conf = points_3d_conf[idx] if points_3d_conf is not None else None
                        intr = intrinsics[idx] if intrinsics is not None and len(intrinsics) == batch_idx.shape[0] else intrinsics
                    else:
                        if points_3d.ndim == 3 and points_3d.shape[0] == bs:
                            pts3d_idx = points_3d[i]
                            pts3d_labels = points_3d_labels[i] if (points_3d_labels is not None and len(points_3d_labels) == bs) else None
                            pts3d_conf = points_3d_conf[i] if (points_3d_conf is not None and len(points_3d_conf) == bs) else None
                            intr = intrinsics[i] if (intrinsics is not None and len(intrinsics) == bs) else intrinsics
                        else:
                            pts3d_idx = points_3d
                            pts3d_labels = points_3d_labels
                            pts3d_conf = points_3d_conf
                            intr = intrinsics
                except Exception:
                    pts3d_idx = points_3d
                    pts3d_labels = points_3d_labels
                    pts3d_conf = points_3d_conf
                    intr = intrinsics
                if isinstance(pts3d_idx, np.ndarray) and pts3d_idx.size:
                    if intr is None:
                        pass
                    else:
                        try:
                            K = np.asarray(intr)
                            if K.ndim == 3 and K.shape[0] == bs and K.shape[1] == 3 and K.shape[2] == 3:
                                K = K[i]
                        except Exception:
                            K = np.asarray(intr)
                        ...
                        for j in range(len(pts3d_idx)):
                            try:
                                if pts3d_conf is not None:
                                    conf_val = float(pts3d_conf[j])
                                    if conf_val < conf_thres:
                                        continue
                                pt = np.asarray(pts3d_idx[j]).astype(float)
                                if pt.size < 7:
                                    continue
                                dims = pt[0:3].tolist()
                                loc = pt[3:6].tolist()
                                ...
                                corners_3d = get_3d_box_corners(dims, loc, yaw) # (8, 3)
                                corners_2d = project_to_image(corners_3d, K)
                                if scale < 1:
                                    corners_2d = corners_2d * scale
                                corners_2d[:, 0] += x
                                corners_2d[:, 1] += y

                                clip_margin = 200
                                if np.all((corners_2d[:, 0] > -clip_margin) & (corners_2d[:, 0] < img_w * ns + clip_margin) &
                                          (corners_2d[:, 1] > -clip_margin) & (corners_2d[:, 1] < img_h * ns + clip_margin)):
                                    cls_idx = None
                                    if pts3d_labels is not None:
                                        try:
                                            cls_idx = int(pts3d_labels[j])
                                        except Exception:
                                            cls_idx = None
                                    color = colors(cls_idx if cls_idx is not None else 0)
                                    im = draw_3d_box(im, corners_2d, color, thickness=2)
                                    center = corners_2d.mean(axis=0).astype(int)
                                    label_text = f"{names.get(cls_idx, cls_idx) if (names and cls_idx is not None) else ''}"
                                    depth_text = f"{loc[2]:.1f}m"
                                    text = f"{label_text} {depth_text}".strip()
                                    cv2.putText(im, text, (center[0], center[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                            except Exception:
                                continue
                        annotator.fromarray(im)
