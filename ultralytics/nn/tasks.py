...
class Detection3DModel(DetectionModel):
    """YOLO Detection3DModel model.

    This class extends DetectionModel to handle instance segmentation tasks, providing specialized loss computation for
    pixel-level object detection and 3d detection.

    Methods:
        __init__: Initialize YOLO 3d detection model.
        init_criterion: Initialize the loss criterion for detection.

    Examples:
        Initialize a 3d detection model
        >>> model = Detection3DModel("yolo11n-3d.yaml", ch=3, nc=80)
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="yolo11n-3d.yaml", ch=3, nc=None, verbose=True):
    ...
    def init_criterion(self):
        """Initialize the loss criterion for the Detection3DModel."""
        return E2EDetectLoss3D(self)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26, v10Detect3D}:
                m.legacy = legacy
        elif m in {v10Detect, v10Detect3D}:
